...
const findDistrict = async (id) => prisma.district.findUnique({ where: { id: Number(id) }, include: { restaurants: true } });

exports.getAllDistricts = asyncErrorHandle(async (req, res) => {
  const districts = await prisma.district.findMany({ include: { restaurants: true } });
  res.json(districts);
});
...
exports.updateDistrict = asyncErrorHandle(async (req, res) => {
  const { id } = req.params;
  const { name, description } = req.body;
  const district = await prisma.district.update({ where: { id: Number(id) }, data: { name, description } });
  res.json(district);
});
